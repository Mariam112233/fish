...
{ 
 $output = '';
 $sql = "SELECT * FROM food_data ORDER BY food_data_id ASC";
 $result = $conn->query($sql); 


 foreach($result as $key=>$row)
 {
  $output .= '<option value="'.$row["food_data_id"].'">'.$row["food_data_id"].' '.$row["food_data_name"].'</option>';
 }
 return $output;
}
...
<head>
  ...
  <script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/js/bootstrap.min.js"></script> -->
    ...
    <title>ข้อมูลการสั่งซื้ออาหารปลา</title>
            ...
            <div class="container-fuit">
                <div class="row">
                    <div class="col-2">
                        <?php
                        $food_order_id = 0 ;
                        $sql1 =  mysqli_query($conn, "SELECT * FROM food_order ORDER BY food_order_id DESC LIMIT 1");
                        while($row1 = mysqli_fetch_array($sql1)){
                        $food_order_id = $row1['food_order_id'];
                        }
                        $food_order_id += 1 ;
                        ?>
                        <!-- ======================================= -->
                        <label for="food_order_id" class="lb">รหัสการสั่งซื้อ</label>
                        <input type="text" name="food_order_id" class="contact-form-text" required value=" <?php echo $food_order_id; ?>" />
                        <!-- ======================================= -->
                    ...
                    <div class="col">
                        ...
                        <input autocomplete="off" type="text" id="inputIsValid " name="supplier_food_id" class="contact-form-text" list='list2' required> 
                        ...
                        $result = $conn->query($sql);?> 
                            <datalist id='list2'>
                                <?php foreach($result as $key=>$row) {
                            echo'
                                <option value="'.$row["supplier_food_id"].'.'.$row["supplier_food_name"].'" "</option>
                            ';
                            } ?>
                        </datalist>
...
$(document).ready(function() {
    ...
    $('#add').click(function() {
        ...
        $('#dynamic_field').append('<tr id="row' +
            i +
            '"><td><select name="food_data_id[]" class="form-control food_data_id"><option value="">ป้อนอาหาร </option><?php echo fill_unit_select_box($conn); ?></select></td>'
            ...
            +
            i +'" /></td><td><input type="number" name="order_food_number[]" placeholder="0.00" step="0.01" min="0.01" max="1000" class="form-control order_food_number" id="order_food_number' +
            i +'" /></td><td><input type="text" name="order_food_price[]" placeholder="00.00" class="form-control order_food_price" id="order_food_price' +
